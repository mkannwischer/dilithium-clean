...
void poly_use_hint(poly *b, const poly *a, const poly *h) {
  ...
  for(i = 0; i < N; ++i)
    b->coeffs[i] = use_hint(a->coeffs[i], (uint32_t) h->coeffs[i]);
...
void polyz_unpack(poly *r, const uint8_t *a) {
  ...
  for(i = 0; i < N/4; ++i) {
    ...
    r->coeffs[4*i+0] |= (int32_t)((uint32_t)a[9*i+1] << 8);
    r->coeffs[4*i+0] |= (int32_t)((uint32_t)a[9*i+2] << 16);
    r->coeffs[4*i+0] &= 0x3FFFF;

    r->coeffs[4*i+1]  = a[9*i+2] >> 2;
    r->coeffs[4*i+1] |= (int32_t)((uint32_t)a[9*i+3] << 6);
    r->coeffs[4*i+1] |= (int32_t)((uint32_t)a[9*i+4] << 14);
    r->coeffs[4*i+1] &= 0x3FFFF;

    r->coeffs[4*i+2]  = a[9*i+4] >> 4;
    r->coeffs[4*i+2] |= (int32_t)((uint32_t)a[9*i+5] << 4);
    r->coeffs[4*i+2] |= (int32_t)((uint32_t)a[9*i+6] << 12);
    r->coeffs[4*i+2] &= 0x3FFFF;

    r->coeffs[4*i+3]  = a[9*i+6] >> 6;
    r->coeffs[4*i+3] |= (int32_t)((uint32_t)a[9*i+7] << 2);
    r->coeffs[4*i+3] |= (int32_t)((uint32_t)a[9*i+8] << 10);
